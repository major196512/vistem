...
class PLANTopDown(PLANBase):
    ...
    def forward(self, x):
        ...
        for head_idx in range(self.num_heads):
            ...
            ret = (value * weight.unsqueeze(1)).view(B, -1, H*W)
            ret = nn.functional.fold(ret, bot_feat.shape[-2:], kernel_size=self.erf, padding=(self.erf-1)//2, stride=2)
            results.append(ret)
